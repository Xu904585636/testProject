#!/bin/bash
MainClass="com.kingleadsw.ysm.mtp.controller.MTPApplication"
cd `dirname $0`
BIN_DIR=`pwd`
cd ..
DEPLOY_DIR=`pwd`
...
JAVA_OPS="-Dapp.id=mecom-mtpApplication -Denv=DEV  -Ddev_meta=http://192.168.1.17:8080"
JAVA_DEBUG_OPTS=""
if [ "$1" = "debug" ]; then
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath .:$CONF_DIR:$DEPLOY_DIR/classes:$LIB_JARS $MainClass > $STDOUT_FILE 2>&1 &
